"""
test_asc_memory.py  —  ASC Memory Engine Backtest v1.0
=======================================================
FinFolioX — Phase 26  |  7 Windows × 30 Tickers

Two-part test:

PART 1 — Unit Tests (no live data needed)
  ...
  6. DS usage: v2.3 BUG-2 fix — DS differentiates moderate zone
  7. FDP: inverted LSTM produces measurable dissent sensitivity
  8. Box report: all lines same width (BUG-3 fix)
  9. n<25 saturation: BUG-1 fix — samples 20-24 use MIN+EXTRA guard
  10. Saturation threshold: 0.04 (BUG-4 fix, was 0.02)

PART 2 — Integration Test (live pipeline, 7 windows × 30 tickers)
...
──────────────────────────────
  TEST-FIX-1 (Test 5): Removed wrong assertion that "diverse data → low penalty".
    Independent random agents have MI≈0, so ASC≈1.0 is CORRECT — the engine is
...
def run_unit_tests():
    print("\n" + "=" * 80)
    print("  PART 1 — UNIT TESTS  (synthetic data, no live pipeline)")
    print("=" * 80)
    ...
    # ── Test 2: BUG-4 — Saturation threshold is 0.04 ─────────────────────
    print("\n── Test 2: BUG-4 fix — saturation threshold = 0.04 ───────────")
    check("SATURATION_STD_THRESHOLD == 0.04",
          SATURATION_STD_THRESHOLD == 0.04,
          f"got {SATURATION_STD_THRESHOLD}")

    # ── Test 3: BUG-1 — n<25 fix ─────────────────────────────────────────
    print("\n── Test 3: BUG-1 fix — n<25 uses configurable guard ───────────")
    ...
    # the very first entry was stored — leaving n=1, asc_reliable=False,
    ...
    for i in range(WINDOW_SIZE):
        # Each step differs by 0.002 — passes anti-spam, stays tightly clustered
    ...
    # That is CORRECT engine behavior — it means agents are not sharing
    ...
    # ── Test 6: BUG-2 — DS is used in moderate zone ──────────────────────
    print("\n── Test 6: BUG-2 fix — DS used in moderate zone 0.70-0.85 ─────")
    ...
    print("\n── Test 8: BUG-3 fix — box report uniform line widths ──────────")
    import io, contextlib
    summary = {
        "asc_score": 0.75, "n_samples": 25, "asc_penalty_multiplier": 0.90,
        "asc_quadrant": "MODERATE SYCOPHANCY — low LSTM dominance (−10%)",
    ...
    if failed == 0:
        ...
        print(f"  ⚠️  {failed} FAILURES — check fixes above")
    print(f"  {'═'*50}")
    return failed == 0


# ==============================================================================
# PART 2 — INTEGRATION TEST (live pipeline)
# ==============================================================================
...
def run_integration_test(tech_agent, uncertainty_agent, regime_agent,
                         fusion_agent):
    print("\n" + "=" * 80)
    print("  PART 2 — INTEGRATION TEST  (live pipeline, 7 windows × 30 tickers)")
    print("=" * 80)
    ...
    for s in all_window_stats:
        relf = "✅" if s["reliable"] else "⏳"
        satf = "🟡" if s["saturated"] else "  "
        penf = "✅" if s["penalty"] == 1.0 else "⚠️"
    ...
    if unreliable_windows:
        print(f"  ℹ️  {len(unreliable_windows)} window(s) still warming "
              f"(n={[s['n'] for s in unreliable_windows]}) — "
    ...
    # as a diagnostic only — not a pass/fail assertion.
    #
    ...
    # than any meaningful threshold — checking it would be testing yfinance
    ...
    if bear_stds and non_bear_stds:
        ...
        print(f"       bear_mean={bear_mean:.4f}  non_bear_mean={nonbear_mean:.4f}  "
              f"|diff|={diff:.4f}  "
              f"({'distinct' if diff > 0.01 else 'similar — saturation filter equalises regimes'})")
    else:
        print("  ℹ️  Regime std diagnostic skipped — insufficient reliable windows "
              "in both categories")
...
def main():
    ...
    print("  FINAL VERDICT")
    print("=" * 80)
    print(f"  Unit tests       : {'✅ ALL PASSED' if unit_ok else '⚠️  FAILURES'}")
    print(f"  Integration tests: {'✅ ALL PASSED' if int_ok  else '⚠️  FAILURES'}")
