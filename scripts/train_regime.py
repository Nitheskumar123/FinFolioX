...
==================================
FinFolioX — One-Time Training Script for Hybrid Regime Agent v2.3.1
...
What it does:
  1. Downloads ^GSPC (2003-2024) via yfinance — falls back to synthetic if offline
...
def main():
    print("=" * 60)
    print("  FinFolioX — Hybrid Regime Agent v2.3.1 Training")
    print("=" * 60)
    ...
    try:
        ...
        if len(df) > 200:
            train_df = df
            print(f"   ✅ Downloaded  |  {len(train_df)} trading days")
        else:
            print("   ⚠️  Too few rows — using synthetic data")
    except Exception as e:
        print(f"   ⚠️  Download failed ({type(e).__name__}) — using synthetic data")

    if train_df is None:
        print("   🔄 Generating synthetic OHLCV (2003–2024, seed=42)...")
        train_df = _synthetic_ohlcv(TRAIN_START, TRAIN_END, seed=42)
        print(f"   ✅ Synthetic data ready  |  {len(train_df)} trading days")
    ...
    # ── 4. Sanity check ───────────────────────────────────────
    print("\n🔍 Loading back and running sanity check...")
