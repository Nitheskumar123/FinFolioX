"""
ml_engine/uncertainty_agent.py  —  Uncertainty Agent v2.3
===========================================================
CHANGELOG v2.3:
  ...
  - predict_from_prob() is now the PRIMARY method — all internal callers
...
class UncertaintyAgent:
    ...
    def __init__(self, technical_agent):
        ...
        print("  ✅ UncertaintyAgent  (distance-from-0.5 method)")

    # ------------------------------------------------------------------
    # PRIMARY METHOD — use this everywhere lstm_signal is already computed
    # ------------------------------------------------------------------
    def predict_from_prob(self, lstm_stretched: float) -> tuple:
        ...
        lstm_stretched : float ∈ [0, 1]
            Output of TechnicalAgent.predict() — the stretched LSTM prob.
            Do NOT pass predict_raw() output here.

        Returns
        -------
        mc_mean : float  — same as lstm_stretched (used as fusion lstm_p)
        mc_std  : float  — distance from 0.5 (uncertainty proxy)
        """
    ...
    def predict_with_uncertainty(self, recent_data_df, n_iterations: int = 10) -> tuple:
        """
        Legacy method — calls tech_agent.predict() internally.

        ⚠️  DO NOT USE when lstm_signal has already been computed by the caller.
        ...
        Exception
            ...
            existed in v2.0–v2.2 has been REMOVED — it corrupted fusion by
            feeding a near-zero z-score to the KaggleFusion model on every
            ticker.
        """
        # Guard: scalar / array passed directly — route to primary method.
        ...
        # Normal DataFrame path — compute lstm_stretched then delegate.
