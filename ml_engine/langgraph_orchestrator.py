"""
ml_engine/langgraph_orchestrator.py  —  LangGraph Multi-Agent Orchestrator (v2.3)
===================================================================================
CHANGELOG:
...
class AgentState(TypedDict):
    ...
    regime_label: str
    current_vol: float
    regime_confidence: float         # ← NEW v2.3: hybrid regime confidence
    risk_score: float
    div_status: str
...
class FinFolioGraphOrchestrator:
    ...
    # -------------------------------------------------------------------------
    # NODE 2: Market Context  ← UPDATED v2.3: uses HybridRegimeAgent
    # -------------------------------------------------------------------------
    def node_market_context(self, state: AgentState) -> AgentState:
        ...
        # Use HybridRegimeAgent if available, fall back to rule-only
        ...
        return {
            ...
            "regime_confidence": regime_confidence,   # ← NEW
        ...
        try:
            ...
            if result is not None:
                if isinstance(result, tuple):
                    ...
                    sent_label = "bullish" if sent_score > 0.07 else "bearish" if sent_score < -0.07 else "neutral"
            else:
        ...
        if self.causal_agent and hist_data is not None:
            try:
                ...
                causal_result = self.causal_agent.analyze(
                    ticker=ticker, target_hist_df=hist_data, universe_data=universe_data,
                )
            except Exception as e:
    ...
    def node_counterfactual_debate(self, state: AgentState) -> AgentState:
        ...
        if causal_score >= 0.65:
            ...
            verdict = f"CAUSAL_WARNED -- {signal_dir} confounder-driven ({len(confounders)}/{total_universe})."
        else:
    ...
    # -------------------------------------------------------------------------
    # NODE 5: Fusion Engine  ← UPDATED v2.3: applies regime_confidence
    # -------------------------------------------------------------------------
    def node_fusion_engine(self, state: AgentState) -> AgentState:
        ...
        lstm_regime_agree_bull = (lstm_signal > 0.58 and state["regime_label"] == "Bull" and sent_score > 0.03)
        lstm_regime_agree_bear = (lstm_signal < 0.42 and state["regime_label"] == "Bear" and sent_score < -0.03)
        ...
        # Apply hybrid regime confidence ← NEW v2.3
        regime_confidence = state.get("regime_confidence", 0.8)
        final_conf = float(np.clip(final_conf * regime_confidence, 0.0, 1.0))
        print(f"      - Regime confidence: {regime_confidence:.2f}x → Fused Confidence (pre-ASC): {final_conf:.4f}")
    ...
    def node_asc_check(self, state: AgentState) -> AgentState:
        ...
        try:
            ...
            self.asc_memory.record_session(
                lstm_score=lstm_signal, sent_score=sent_score, regime_prob=regime_prob,
            )
            ...
            if regime_label == "Bull" and working_conf < REGIME_CONTRADICTION_CONF_THRESHOLD:
                ...
                print(f"      [ASC] REGIME CONTRADICTION (Bull+LowConf): {old_conf:.3f} → {working_conf:.3f}")

            elif regime_label == "Sideways" and (working_conf > 0.70 or working_conf < 0.35):
                regime_contradiction = True
                old_conf     = working_conf
                working_conf = 0.50 + (working_conf - 0.50) * 0.50
                print(f"      [ASC] REGIME CONTRADICTION (Sideways+Directional): {old_conf:.3f} → {working_conf:.3f}")
                ...
                working_conf = float(np.clip(working_conf, 0.0, 1.0))
                print(f"      [ASC] REGIME CONTRADICTION (Bear+Bullish): {old_conf:.3f} → {working_conf:.3f}")
            ...
            print(f"      - ASC Score: {asc_score:.4f} (reliable: {asc_reliable}, saturated: {asc_saturated})")
            ...
            if asc_reliable and not asc_saturated and asc_score >= 0.85:
                ...
                fdp_result = self.asc_memory.run_forced_dissent(
                    lstm_signal=lstm_signal, sent_score=sent_score,
                    regime_label=regime_label, fusion_agent=self.master.fusion_agent,
            ...
            if penalty < 1.0:
                print(f"      - Confidence: {working_conf:.4f} → {adjusted_conf:.4f} (ASC penalty)")

            effective_threshold = (
                COMMODITY_BUY_THRESHOLD if state.get("ticker", "") in COMMODITY_TICKERS
            ...
            summary = self.asc_memory.get_asc_summary(
                asc_result=asc_result, fdp_result=fdp_result, penalty=penalty, quadrant=quadrant,
            )
    ...
    def node_conflict_resolution(self, state: AgentState) -> AgentState:
        ...
        if self.master.conflict_resolver:
            arbitration_result = self.master.conflict_resolver.arbitrate(
                tech_score=state["lstm_signal"], sent_score=state["sent_score"],
                mc_std=state["mc_std"], regime_label=regime_label,
                risk_score=state["risk_score"], fusion_confidence=fusion_conf,
                trust_scores=state.get("trust_scores", None),
            )
        ...
        if hasattr(self.master, "heatmap_agent") and self.master.heatmap_agent:
            heatmap_result = self.master.heatmap_agent.analyze(
                lstm_score=state["lstm_signal"], sent_score=state["sent_score"],
                regime_label=regime_label, regime_vol=state.get("current_vol", 0.5),
            )
        ...
        causal_mod_gdi, gdi_causal_penalty = _CausalAgent.apply_gdi_penalty(raw_causal_mod, gdi)
        topo_mod          = state.get("topology_modifier", 1.0)
        combined_modifier = (topo_mod + causal_mod_gdi) / 2.0

        if gdi_causal_penalty > 0.0:
            print(f"      [Conflict] GDI={gdi:.3f} → causal_mod {raw_causal_mod:.3f}→{causal_mod_gdi:.3f}")
            adj_conf = float(np.clip(adj_conf * combined_modifier, 0.0, 1.0))
            print(f"      [Conflict] Combined modifier {combined_modifier:.3f}x → adj_conf={adj_conf:.4f}")

        last_price = state["hist_data"]["Close"].iloc[-1]
        alloc_pct, _ = self.master.risk_engine.calculate_position_size(
            adj_conf, state["current_vol"], disagreement_penalty=gdi_penalty, regime=regime_label,
        )
    ...
    def node_red_team(self, state: AgentState) -> AgentState:
        ...
        if "BUY" not in state["final_decision"]:
            return {"red_team_passed": True, "red_team_delta": 0.0, "final_decision": state["final_decision"]}

        if self.master.red_team:
            try:
                crashed_df    = self.master.red_team.generate_flash_crash(state["hist_data"], drop_pct=0.10)
                ...
                return {"red_team_passed": True, "red_team_delta": delta, "final_decision": state["final_decision"]}
            except Exception as e:
    ...
    def node_llm_supervisor(self, state: AgentState) -> AgentState:
        ...
        if state.get("asc_reliable"):
            ...
            if state.get("fdp_ran"):
                asc_note += f"\nForced Dissent: Sensitivity={state.get('dissent_sensitivity', 0.0):.3f}"
        ...
        # Include hybrid regime confidence in summary
...
"""
        ...
        if hasattr(self.master, "meta_agent") and self.master.meta_agent:
            try:
                ...
                self.master.meta_agent.log_decision(
                    ticker=state["ticker"], lstm_score=state["lstm_signal"],
                    sent_score=state["sent_score"], regime_label=state["regime_label"],
                    risk_score=state["risk_score"], fusion_confidence=state["fusion_confidence"],
                    final_decision=state["final_decision"], price_at_decision=float(last_price),
                    asc_score=state.get("asc_score", 0.5), asc_reliable=state.get("asc_reliable", False),
                )
            except Exception as e:
    ...
    def run_analysis(self, ticker: str):
        initial_state = {
            ...
            "sent_bias_warning":      False,
            "sent_label":             "neutral",
            "regime_confidence":      0.8,        # ← NEW default
            "asc_score":              0.5,
            "asc_reliable":           False,
            "asc_saturated":          False,
            "asc_penalty_multiplier": 1.0,
        ...
        print(f"\n[LangGraph Orchestrator v2.3] Starting 12-node graph for {ticker}...")
        ...
        print("\n" + "=" * 72)
        print(f"FINAL DECISION: {final_state.get('final_decision')}{bias_flag}{regime_flag}{sat_flag}")
        print(
